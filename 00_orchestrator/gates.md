...
| G-06 | after pre-review or reviewer response | entry was 06; inventable items are questions to the user; 选刊 not sent to `05-write-venue`; undecidable items include reference-only sentence drafts, not silent manuscript edits; **if this run mounted ARS/MedSci/Scientific/OpenClaw/non-personal B, at least one Word comment prefix must name that source** (`[ARS:…]` / `[MedSci:…]` / `[Scientific:…]` / `[OpenClaw:…]` / `[B:…]`) — else fail and re-tag prefixes only | 06 Lee |
