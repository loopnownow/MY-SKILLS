...
5. **Missing-value handling (`02_data-processing`, mounted `clean-data` when picked) then pipeline (`04_analysis`)** — drop >50% missing; `python -m modules.pipeline`. `VAL_MODE` and columns only from `settings.ini`.
