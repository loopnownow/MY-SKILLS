...
1. **Detect 定入口与终点**——已有 HTML 可停在 05，不必强进 06。结果页和审稿信同时在、且没有点名入口时，先问从哪一站进。`结束` 或本次文件已在且最后一门 PASS → `pipeline.stage: done`。暂停，或停在结果页后 / 预审前，stage 留在当前站。同一缺陷第 3 轮仍失败：`unresolved`，stage 不往前。同一次已批准运行里，后续过站报一行；患者信息、结果页数字、预审入口、审稿门仍停住。
