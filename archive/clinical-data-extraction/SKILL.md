...
7. **免疫组化**：从病理**全文**抽（`指标（结果）`）。结构化 `病理_ER` 等列若过工作区缺失阈值（见 `core/01_automation/references/0rad-workspace.md`），不要整列写入，改从全文解析。
8. **阴性不是缺失**：括号里的 `-` / `－` / `—` / `阴性` 编码为 `Negative`，不得当空值丢掉。
9. **标记名完整匹配**：`ER` 不得命中 `HER2`；在标记名前要求非字母（或词界）。`P16` 的斑驳/斑片单独记 `Mosaic`，不要并进 `Positive`。
10. 分类标签用英文（`Positive`/`Negative`），规则见 `core/01_automation/references/0rad-workspace.md`，此处不另写一套。
