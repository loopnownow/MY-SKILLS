...
**粗 ID 10 个（焊死 stage buckets）+ 细 ID 52 个（session-pick；reference_only 已空）。** Hybrid mount：不同细 ID 可用不同包；同一细 ID 内不混包。备份源仍 PROPOSED。OpenClaw **不得**作为任何细 ID 的 atomic source。
