...
| Excel/CSV, 0RAD workspace, batch tables, imaging prep, missing-value handling via `clean-data` | `02_data-processing` | Soft-coding → `02_data-processing/code-refactoring`; modeling → `04_analysis` |
...
| Clinical text / HIS / pathology extraction | `02_data-processing/clinical-data-extraction` | Missing-value handling → `02_data-processing` (`clean-data`) |
...
| Study design, **literature**, grants | `03_research` | 选刊 → `03_research`; I/D wording → `05_manuscript`; 样本量 → `04_analysis` (`calc-sample-size`) |
...
- “画 ROC/流程图” → `04_analysis` (mounted `fig-plot` / `make-figures`). 选刊 → `03_research`. 样本量 → `calc-sample-size`.
