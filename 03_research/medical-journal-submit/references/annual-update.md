...
2. Whether blacklist / layers / 每层5本 changed. If they say nothing, keep `references/policy.md`.
...
- layer CSVs in this `references/` folder
- `category-index.csv`, `full-pool.csv`
- `artifacts/黑名单_默认挂载.csv` (names stay; metrics may change)

Rebuild `artifacts/医学投稿推荐_JCR20XX.xlsx` and layer CSVs from the raw workbook, then **discard** the raw Clarivate file from this pack (user keeps a private copy if needed). Keep the curated xlsx + derived CSVs.
...
Exact journal lists, APC numbers, article counts, IF values. Those belong in the yearly tables.
