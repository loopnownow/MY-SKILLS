...
3. Vancouver order of first appearance. Do not cite every sentence. Neighbouring sentences that share one source: cite once. Do not reuse an earlier number after a later number has entered (no `[1]` after `[3]`). Intro last paragraph and Discussion first paragraph: **no citations**. Methods: **no citations**. Quotas and placement: **`Aitor-format.md`**. Avoid `[1–3]` clusters.  
