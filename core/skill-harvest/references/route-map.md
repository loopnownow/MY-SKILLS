...
| Excel/CSV, 0RAD workspace, batch tables, imaging prep, impute | `02_data-processing` | Soft-coding → `code-refactoring`; modeling → `04_analysis` |
| Soft-coding, dry-run, CONFIG-on-top refactor | `code-refactoring` | Statistical methods → `04_analysis`; imaging prep → `02_data-processing` |
| Clinical text / HIS / pathology extraction | `clinical-data-extraction` | Imputation → `02_data-processing` (`02-impute`) |
...
| Clinical translation / reader studies | `clinical-translation` | Stats → `04_analysis`; figures → `04_analysis` |
...
- “处理 Excel / 0RAD 文件夹” → `02_data-processing`; “修 Python/软编码” → `code-refactoring`.
