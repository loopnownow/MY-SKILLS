...
A = framework + personal. B = default mounted source. Registry (v4, `01_skill-discovery-integration/registry.yaml`) is **10 coarse ids (welded stage buckets) → 55 fine ids (session-pick mount points)**, not a flat menu. `session_mount: ask-each-run`, `session_pick_unit: fine_id`. Pick fine ids each run; do not auto-load all. Never auto-mount a non-B source. ARS/MedSci/Scientific/AIPOCH/Nature stay PROPOSED backups; OpenClaw is never an atomic mount source.
